function getFormattedTimeData(data) {
   ...
   const regexElapsedTime = /Elapsed \(wall clock\) time \(h:mm:ss or m:ss\):\s*(.*?)/;
   const regexAvgSharedTextSize = /Average shared text size \(kbytes\):\s*(\d+)/;
   const regexAvgUnsharedDataSize = /Average unshared data size \(kbytes\):\s*(\d+)/;
   const regexAvgStackSize = /Average stack size \(kbytes\):\s*(\d+)/;
   const regexAvgTotalSize = /Average total size \(kbytes\):\s*(\d+)/;
   const regexMaxResidentSetSize = /Maximum resident set size \(kbytes\):\s*(\d+)/;
   const regexAvgResidentSetSize = /Average resident set size \(kbytes\):\s*(\d+)/;
   const regexMajorPageFaults = /Major \(requiring I\/O\) page faults:\s*(\d+)/;
   const regexMinorPageFaults = /Minor \(reclaiming a frame\) page faults:\s*(\d+)/;
   const regexVoluntaryContextSwitches = /Voluntary context switches:\s*(\d+)/;
   const regexInvoluntaryContextSwitches = /Involuntary context switches:\s*(\d+)/;
   ...
   const voluntaryContextSwitchesMatch = data.match(regexVoluntaryContextSwitches);
   if (voluntaryContextSwitchesMatch) {
       timeData.voluntaryContextSwitches = parseInt(voluntaryContextSwitchesMatch[1]);
   }
   
   const involuntaryContextSwitchesMatch = data.match(regexInvoluntaryContextSwitches);
   if (involuntaryContextSwitchesMatch) {
       timeData.involuntaryContextSwitches = parseInt(involuntaryContextSwitchesMatch[1]);
   }
